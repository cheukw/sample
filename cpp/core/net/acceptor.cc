...
Acceptor::Acceptor(struct event_base* ev_base, const std::string& ip, uint16_t port, AcceptCallbacksPtr cb, ConnCallbacksPtr lkcb)
	: ev_base_(ev_base)
	, listener_(nullptr)
	, accept_cbs_(cb)
	, conn_cbs_(lkcb)
...
void Acceptor::AcceptCallback(evconnlistener * listener, evutil_socket_t fd, sockaddr* addr, int len, void* ctx) 
{
	...
	auto conn = std::make_shared<Connection>(self->ev_base_, fd);
	conn->SetCallback(self->conn_cbs_);
	conn->Enable();
